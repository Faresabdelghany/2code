...
import type { ProcessContent, ProcessItem } from "@/lib/types/cms";
...
interface Props {
  ...
  onChange: (c: ProcessContent) => void;
}

export default function ProcessEditor({ content, onChange }: Props) {
  function updateItem(index: number, updates: Partial<ProcessItem>) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <span style={{ ...labelStyle, marginBottom: 0 }}>Steps ({content.items.length})</span>
        <button
          onClick={addItem}
      ...
      {content.items.map((item, i) => (
        <div key={i} style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ color: "#f0ebe3", fontSize: 13, fontWeight: 600 }}>Step {i + 1}</span>
            <button
          ...
          <RichTextField label="Title" value={item.title} onChange={(html) => updateItem(i, { title: html })} />
          <RichTextField label="Description" value={item.description} onChange={(html) => updateItem(i, { description: html })} />
        </div>
      ))}
    </div>
  );
